...
import 'package:toastification/toast_src/built_in/built_in_builder.dart';
import 'package:toastification/toast_src/core/toastification_manager.dart';
...
/// Both [show] and [showCustom] methods now accept an optional [config] parameter
/// that allows you to override global configuration for individual toasts.
///
/// ### Using config parameter with showCustom:
/// ```dart
/// toastification.showCustom(
///   config: ToastificationConfig(
///     itemWidth: 550,                    // Override width constraint
///     margin: EdgeInsets.all(20),        // Simple margin (priority over marginBuilder)
...
/// ```
///
/// ### Using config parameter with show:
/// ```dart
/// toastification.show(
///   config: ToastificationConfig(
///     itemWidth: 400,
...
///   autoCloseDuration: Duration(seconds: 3),
///   config: ToastificationConfig( // optional config parameter
///     itemWidth: 550,
...
class Toastification {
  ...
  ToastificationItem showCustom({
    ...
    /// find the config from the provided parameter, context, or use the global config
    final ToastificationConfig effectiveConfig = config ??
        (contextProvided
    ...
    final effectiveAlignment =
        (alignment ?? effectiveConfig.alignment).resolve(direction);
  ...
  ToastificationItem show({
    ...
    if (closeButtonShowType != null && closeButtonShowType != closeButton.showType) {
